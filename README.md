...
- **S3 업로드 방식**: 프론트에서 presigned URL로 직접 업로드할지, 백엔드를 경유해서 업로드할지.
...
- **다국어(한국어/English/日本語)**: 우측 상단 토글로 전환하며 `react-i18next` 기반. UI 텍스트(버튼, 안내 문구, 캘린더 요일/월 표기 등)만 번역되고, 사용자가 직접 입력한 항목 제목/메모/커스텀 단계 이름은 번역되지 않습니다. 예외적으로 앱이 자동 생성하는 시작/완료 단계 라벨은 `isDefault: true`인 동안만 언어에 따라 번역되어 보이며, 사용자가 그 이름을 한 번이라도 수정하면 `isDefault`가 `false`로 바뀌면서 이후로는 입력한 텍스트 그대로 고정됩니다.
- 사진/첨부파일은 최종적으로 AWS S3에 저장하고 DB에는 S3 URL만 저장할 예정입니다. 다만 현재는 사용자 요청으로 업로드 UI/UX를 먼저 검증하는 단계라 프론트엔드에서 `URL.createObjectURL`로 만든 로컬 blob URL을 컴포넌트 상태에만 담아 미리보기만 제공하고 있습니다(새로고침하면 사라지고, 서버/DB에는 저장되지 않음). S3 업로드 방식이 정해지면 이 부분을 실제 연동으로 교체해야 합니다.
